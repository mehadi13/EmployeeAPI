...
public class MaritalStatusRestController {
	...
	public ResponseEntity<ApiResponse> findAll(){
		...
		if (maritalstatus != null) {
		    return new ApiResponse(ClassWrapper.getWrapper("maritalstatuses", maritalstatus))
		    		.send(HttpStatus.OK,ReturnCode.SUCCESS,"Successful");
	        } else {
            return new ApiResponse().send(HttpStatus.NOT_FOUND, "Marital Status List not found!");
        }
	}
	...
	public ResponseEntity<ApiResponse>  getOne(@PathVariable Integer id){
		...
		if (maritalStatus != null) {
            return new ApiResponse(ClassWrapper.getWrapper("maritalstatus", maritalStatus))
            		.send(HttpStatus.OK,ReturnCode.SUCCESS,"Successful");
        } else {
            return new ApiResponse().send(HttpStatus.NOT_FOUND, "Marital Status not found!");
        }
	}
}
